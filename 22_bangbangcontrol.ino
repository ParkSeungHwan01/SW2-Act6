...
void loop()
{
  ...
  if (event_dist) {
    ...
    // bang bang control
  if  (dist_ema > _DIST_TARGET + _BANGBANG_RANGE) {
      duty_target = _DUTY_NEU - _BANGBANG_RANGE;
      digitalWrite(PIN_LED, 0);
    } else if (dist_ema < _DIST_TARGET - _BANGBANG_RANGE) {
      duty_target = _DUTY_NEU + _BANGBANG_RANGE;
...
float volt_to_distance(int a_value)
{
  ...
  return 948 +( -5.99*a_value) + (0.015*pow(a_value, 2)) + (-1.4*pow(10, -5)*pow(a_value, 3));
}
